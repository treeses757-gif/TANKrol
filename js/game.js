...
const BASE_SPEED = 1.5; // Умеренная скорость
let lobbyScreenEl, gameScreenEl;

let lastSendTime = 0;
const SEND_INTERVAL = 50;

// Флаг для предотвращения наложения баффов скорости
...
export function initGame(components) {
    ...
    // Баффы появляются каждые 10 секунд
...
function applyPowerup(type, forPlayer) {
    if (!forPlayer) return; // пока игнорируем баффы для врага
