...
export function createSelectionScreen(onConfirm) {
    ...
    const existing = document.getElementById('selection-screen');
    ...
    tanksContainer.style.maxWidth = '90%';
    ...
    tankList.forEach(tankId => {
        ...
        card.innerHTML = `
            ...
            <div style="font-size: 12px; opacity: 0.9;">${tank.description}</div>
        `;
...
export function showWaitingMessage() {
    ...
    const existing = document.getElementById('waiting-message');
    if (existing) existing.remove();
